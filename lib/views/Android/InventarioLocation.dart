...
class AndroidInventarioLocationPage extends StatefulWidget {
  AndroidInventarioLocationPage({
    Key key,
  ...
  final String sede;
  @override
  AndroidInventarioLocationPageState createState() =>
      AndroidInventarioLocationPageState();
}

class AndroidInventarioLocationPageState
    extends State<AndroidInventarioLocationPage> with TickerProviderStateMixin {
  List<Locations> locations = [];
  // Define an async function to initialize FlutterFire
  void initializeFlutterFire() async {
    if (kIsWeb) {
      // print(['Flutter Web', widget.location.location]);
  ...
  /* getData(CollectionReference collection, String box, String plat,
    ...
    try {
      ...
      switch (widget.location.location) {
        case 'inventario':
          local['nombres']['sedes'][widget.location.sede.nombre]['ubicaciones']
                  [widget.location.ubicacion.nombre]['subUbicaciones']
              [widget.location.subUbicacion.nombre]['inventario'] = {};

          local['keys']['sedes'][widget.location.sede.key]['ubicaciones']
                  [widget.location.ubicacion.key]['subUbicaciones']
              [widget.location.subUbicacion.key]['inventario'] = {};
          await collection.get().then((QuerySnapshot querySnapshot) => {
                querySnapshot.docs.forEach((doc) {
                  // print(doc.data());
                  var loc = Locations.fromFirebase(doc.data());
                  loc.sede = widget.location.sede;
                  loc.ubicacion = widget.location.ubicacion;
                  loc.subUbicacion = widget.location.subUbicacion;
                  // print(['loc', loc.toJson()]);
                  array.add(loc);
                  locationst.add(loc.toJson());
                  local['nombres']['sedes'][widget.location.sede.nombre]
                                      ['ubicaciones']
                                  [widget.location.ubicacion.nombre]
                              ['subUbicaciones']
                          [widget.location.subUbicacion.nombre]['inventario']
                      [doc.data()['nombre']] = {};
                  local['keys']['sedes'][widget.location.sede.key]
                              ['ubicaciones'][widget.location.ubicacion.key]
                          ['subUbicaciones'][widget.location.subUbicacion.key]
                      ['inventario'][doc.data()['key']] = {};
                  ...
                  local['keys']['sedes'][widget.location.sede.key]
                              ['ubicaciones'][widget.location.ubicacion.key]
                          ['subUbicaciones'][widget.location.subUbicacion.key]
                      ['inventario'][doc.data()['key']] = doc.data();
                })
              });
  ...
  getInventario([String subUbicacion, String ubicacion, String sede]) async {
    // await FirebaseFirestore.instance.collectionGroup('inventario');
    try {
      await FirebaseFirestore.instance
          .collectionGroup('inventario')
          .where('sede', isEqualTo: widget.sede)
      ...
      print(['Error collectionGroup query', e]);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    print('AndroidInventarioLocationPage');
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        FittedBox(
                          child: Text(
                            'Sede ', //+
                            // widget.location.sede.nombre.capitalize() +
                            // ' - ' +
                            // widget.location.ubicacion.nombre.capitalize(),
                            style: TextStyle(
                              fontWeight: FontWeight.bold,
                              fontSize: fontSize,
